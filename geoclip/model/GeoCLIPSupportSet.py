...
class SupportSetDataStructure:
    ...
    @torch.no_grad()
    def __init__(self, support_set_loader, image_encoder, location_encoder, batch_size, support_size, device="cuda:0"):
        embedding_size = 512
        ...
        for i, (imgs, gps) in enumerate(support_set_loader):
            ...
            img_features_view_1 = image_encoder(imgs[:, 0])
...
class GeoCLIPSupportSet(nn.Module):
    def __init__(self, support_set_loader, batch_size: int, from_pretrained=True, queue_size=128, support_size=2048, device="cuda:0"):
        super().__init__()
        ...
        self.support_set = SupportSetDataStructure(support_set_loader, self.image_encoder, self.location_encoder, batch_size, support_size)
    ...
    def append_support_set_features(self, img_emb: torch.Tensor, gps_emb: torch.Tensor, gps_coords: torch.Tensor):
        """
        ...
        self.support_set.img_features[:, ptr:ptr + gps_batch_size] = img_emb
        self.support_set.gps_features[:, ptr:ptr + gps_batch_size] = gps_emb
        self.support_set.gps[:, ptr:ptr + gps_batch_size] = gps_coords
        ptr = (ptr + gps_batch_size) % self.support_set_size  # move pointer
